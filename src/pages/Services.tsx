  ...
  <div>
    <section className="hero-gradient px-4 py-20 text-center text-primary-foreground">
      ...
      <p className="mt-3 text-primary-foreground/70">
        Comprehensive coaching for every student's needs
      </p>
    </section>
    ...
    <section className="section-padding bg-background">
      <div className="mx-auto max-w-6xl space-y-8">
        {services.map((s, i) => (
          <div
            key={s.title}
            className={`grid gap-8 rounded-2xl border bg-card p-8 card-shadow lg:grid-cols-[1fr_2fr] ${
              i % 2 !== 0 ? "lg:grid-cols-[2fr_1fr]" : ""
            }`}
          >
            <div
            ...
            <div className={`${i % 2 !== 0 ? "lg:order-1" : ""}`}>
              <div className="grid gap-3 sm:grid-cols-2">
                {s.features.map((f) => (
                  <div
                    key={f}
                    className="flex items-center gap-3 rounded-xl bg-primary/5 p-4"
                  >
